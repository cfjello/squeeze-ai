...
*Source: `parser_v15_scope.go`, spec `07_types_scope.sqg`*

| Type | Fields | Grammar rule |
|------|--------|--------------|
| `V13ScopeInjectNode` | `Binds []V13ScopeInjectBind` | `scope_inject` (`(…)`) |
| `V13ScopeAssignInlineNode` | *(no fields)* | `scope_assign_inline` (`_`) |
| `V13ImportAssignNode` | `LHS V13Node` · `Targets []V13Node` | `import_assign` |
| `V13OtherInlineAssignNode` | `Oper *V13AssignOperNode` · `RHS V13Node` | `other_inline_assign` |
| `V13ScopeBodyItemNode` | `Value V13Node` | one item in a scope body |
| `V13PrivateItemNode` | `Item *V13ScopeBodyItemNode` | `private_item` (`-` single item) |
| `V13PrivateBlockNode` | `UseParen bool` · `Items []*V13ScopeBodyItemNode` | `private_block` (`-( )` or `-{ }`) |
...
| `V13ScopeWithCatchNode` | `Scope *V13ScopeAssignNode` · `Handler *V13FuncUnitNode` | `scope_with_catch` |
...
| `V13ReceiverClauseNode` | `Name string` · `Type *V13InspectTypeNode` | `receiver_clause` (`( ident_name inspect_type )`) |
| `V13ReceiverMethodAssignNode` | `Receiver *V13ReceiverClauseNode` · `LHS *V13AssignLHSNode` · `Oper *V13AssignOperNode` · `RHS *V13AssignRHSNode` · `Body []*V13ScopeBodyItemNode` · `CatchHandler *V13FuncUnitNode` | `receiver_method_assign` |

`V13ImportAssignNode.LHS` is `*V13AssignLHSNode` (named import) or `*V13ScopeAssignInlineNode` (wildcard `_` import).  
`V13ImportAssignNode.Targets` is a slice; each element is `*V13URLNode` or `*V13FileURLNode`.  
`V13ScopeBodyItemNode.Value` is one of: `*V13ImportAssignNode`, `*V13AssignmentNode`, `*V13ScopeAssignNode`, `*V13ScopeWithCatchNode`, `*V13ReceiverMethodAssignNode`, `*V13PrivateItemNode`, `*V13PrivateBlockNode`.  
`V13ReceiverMethodAssignNode`: `Func` is non-nil when the RHS is a `func_unit`; `Body` is non-nil when the RHS is a `{ ... }` scope body.  
`V13ParserRootNode.Value` is `*V13ImportAssignNode`, `*V13ScopeAssignNode`, or `*V13ScopeWithCatchNode`.  
`V13PrivateBlockNode.Items[i].Value` follows the same union as `V13ScopeBodyItemNode.Value` excluding `*V13PrivateItemNode` and `*V13PrivateBlockNode` (no nesting).

---

## 8. §08 Ranges & validation

*Source: `parser_v15_range.go`, spec `08_range.sqg`*
